...
class BoxTokenParser extends AbstractTokenParser
{
    public function parse(Token $token): Node
    {
        ...
        $left = $parser->getExpressionParser()
            ->parseExpression();
        $tokenStream->expect(Token::PUNCTUATION_TYPE, ',');
        $top = $parser->getExpressionParser()
            ->parseExpression();
        if ($tokenStream->nextIf(Token::PUNCTUATION_TYPE, ',')) {
            $width = $parser->getExpressionParser()
                ->parseExpression();
        }

        if ($tokenStream->nextIf(Token::PUNCTUATION_TYPE, ',')) {
            $height = $parser->getExpressionParser()
                ->parseExpression();
        }

        if ($tokenStream->nextIf(Token::NAME_TYPE, 'with')) {
            $attributes = $parser->getExpressionParser()
                ->parseExpression();
        }
        ...
        if ($tokenStream->nextIf(Token::NAME_TYPE, 'as')) {
            ...
            if ($tokenStream->nextIf(Token::NAME_TYPE, 'with')) {
                $table_attributes = $parser->getExpressionParser()
                    ->parseExpression();
            }
        }
